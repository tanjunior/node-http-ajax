...
const upload = async (e) => {
    ...
    if (data.sid == null || data.sid.length == 0) {
        showAlert("Student ID cannot be empty!")
        return false
    } else if (data.fname == null || data.fname.length == 0 || hasNumbers(data.fname)) {
        showAlert("First Name cannot be empty or contains numbers!")
        return false
    } else if (data.lname == null || data.lname.length == 0 || hasNumbers(data.lname)) {
        showAlert("Last Name cannot be empty or contains numbers!")
        return false
    } else if (data.age == null || data.age.length == 0 || data.age < 1) {
        showAlert("Age cannot be empty or less than 1!")
        return false
    } else if (data.gender == null || data.gender.length == 0) {
        showAlert("Gender not selected!")
        return false
    } else if (data.degree == null || data.degree.length == 0) {
        showAlert("Degree cannot be empty!")
        return false
    }
